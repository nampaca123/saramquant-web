...
import type { PathPercentilePoint, Language, LocalizedText } from '@/types';
...
export interface SimulationResult {
  ...
  var: number;
  cvar: number;
  pathPercentiles: PathPercentilePoint[];
  finalPercentiles: Record<string, number>;
  simulationDays: number;
}

export interface SimulationSectionProps {
  onRun: (params: SimulationParams) => Promise<SimulationResult>;
  referenceLine?: number;
  title: LocalizedText;
  description: LocalizedText;
  howItWorksDetail: LocalizedText;
  defaultMethod?: 'gbm' | 'bootstrap';
  chartPreviewText?: LocalizedText;
  finalDistributionText?: LocalizedText;
  loadingStages?: LocalizedText[];
  maxWaitText?: LocalizedText;
  methodTooltip?: LocalizedText;
  partialCoverage?: { symbols: string[] } | null;
}

/* ── Constants ── */

const PERCENTILE_LABELS: Record<string, LocalizedText> = {
...
const DEFAULT_STAGES = [t.simulation.simStage1, t.simulation.simStage2, t.simulation.simStage3];
...
export function SimulationSection({
  ...
  howItWorksDetail,
  defaultMethod = 'gbm',
  chartPreviewText = t.simulation.chartPreview,
  finalDistributionText = t.simulation.finalDistribution,
  loadingStages = DEFAULT_STAGES,
  maxWaitText = t.simulation.simMaxWait,
  methodTooltip = t.simulation.methodTooltip,
  partialCoverage,
  ...
  const [method, setMethod] = useState<'gbm' | 'bootstrap'>(defaultMethod);
  ...
  const handleRun = async () => {
    ...
    setError('');
    try {
      const res = await onRun({ days: Number(days), confidence: Number(confidence), method });
      setResult(res);
    } catch {
  ...
  return (
    <Card>
      ...
      <p className="text-xs text-zinc-500 mb-4">{txt(description)}</p>
      ...
      {loading ? (
        ...
        <OutputPreview
          ...
          txt={txt}
          language={language}
          chartPreviewText={chartPreviewText}
          finalDistributionText={finalDistributionText}
        />
      ) : (
        <SimulationResults
          result={result}
...
function MetricCard({ label, value, tooltip, txt }: {
  ...
  return (
    <div className="relative rounded-xl border border-zinc-100 p-3 text-center">
      ...
      <p className={cn('text-lg font-mono font-bold mt-1', value > 0 ? 'text-up' : 'text-down')}>
...
function OutputPreview({ confidenceLabel, txt, language, chartPreviewText, finalDistributionText }: {
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="flex flex-col items-center justify-center rounded-xl border border-dashed border-zinc-200 bg-zinc-50/50 py-10">
        ...
        <p className="text-xs text-zinc-400">{txt(chartPreviewText)}</p>
      </div>

      <div className="rounded-xl border border-dashed border-zinc-200 p-3">
        <p className="text-[11px] text-zinc-400 mb-2 font-medium">{txt(finalDistributionText)}</p>
...
function SimulationResults({
  ...
  const metrics = [
    { label: txt(t.simulation.expectedReturn), value: result.expectedReturn, tooltip: RESULT_TOOLTIPS.expectedReturn },
  ...
  const sortedPercentiles = Object.entries(result.finalPercentiles).sort(
  ...
  return (
    <div className="space-y-4">
      ...
      {result.pathPercentiles.length > 0 && (
        <SimulationFanChart
          data={result.pathPercentiles}
          referenceLine={referenceLine}
        />
      )}
      ...
      {sortedPercentiles.length > 0 && (
        <div className="rounded-xl border border-zinc-100 p-3">
          <p className="text-[11px] font-medium text-zinc-500 mb-2">
            {txt({
              ko: `${result.simulationDays}일 후 예상 분포`,
              en: `Distribution after ${result.simulationDays} days`,
            })}
          </p>
          <div className="space-y-1.5">
            {sortedPercentiles.map(([pct, value]) => {
              const change = referenceLine ? ((value - referenceLine) / referenceLine) * 100 : null;
              return (
                <div key={pct} className="flex items-center justify-between text-xs">
      ...
      {partialCoverage && partialCoverage.symbols.length > 0 && (
        <p className="text-xs text-caution">
          {txt(t.simulation.partialCoverage)}: {partialCoverage.symbols.join(', ')}
        </p>
      )}
